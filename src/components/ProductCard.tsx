...
export default function ProductCard({
  ...
  return (
    <Link
      href={`/products/${slug}`}
      className="group flex flex-col bg-white rounded-2xl border border-neutral-100 overflow-hidden shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300"
    >
      ...
      <div className="relative aspect-[4/3] w-full bg-neutral-50 overflow-hidden">
        ...
        <button
          ...
          className="absolute top-2.5 right-2.5 w-8 h-8 rounded-full bg-white/90 backdrop-blur-sm shadow flex items-center justify-center text-gray-400 hover:text-red-500 hover:scale-110 transition-all opacity-0 group-hover:opacity-100"
        >
          <Heart
            className={clsx("w-3.5 h-3.5", isLiked && "fill-red-500 text-red-500")}
          />
        </button>
      ...
      <div className="p-4 flex-1 flex flex-col justify-between">
        <div>
          {/* Brand */}
          {brand && (
            <p className="text-[10px] font-semibold text-[var(--color-gold)] uppercase tracking-wider mb-1">
              {brand}
            </p>
          )}
          ...
          {/* Title */}
          <h3 className="text-sm font-semibold text-neutral-800 line-clamp-1 group-hover:text-[var(--color-gold)] transition-colors">
            {name}
          </h3>

          {/* Description */}
          <p className="text-xs text-neutral-400 mt-1 line-clamp-2 min-h-[32px] leading-relaxed">
            {shortDescription ||
          ...
          {/* Weight */}
          {weight && (
            <p className="text-[10px] text-gray-400 mt-1 font-medium">{weight}</p>
          )}
        </div>
